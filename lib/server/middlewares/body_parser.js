...
class BodyParser
{
	...
	constructor( event, options )
	{
		...
		this.dieOnError	= options.dieOnError || true;
		this.event		= event;
		this.rawPayload	= [];
		this.parsers	= [];
	...
	attachEvents( onDataCallback, onEndCallback )
	{
		this.event.request.on( 'data', ( data ) =>
		{
			if ( ! this.event.isFinished() )
			{
				...
				onDataCallback( this.rawPayload );
			}
		});
		...
		this.event.request.on( 'end', () => {
			if ( ! this.event.isFinished() )
			{
				this.rawPayload	= Buffer.concat( this.rawPayload );
		...
		for ( let index in this.parsers )
		{
			...
			if ( parser.supports( this.event ) )
			{
				parser.parse( this.event, ( err ) =>{
					...
					{
						this.event.setError( 'Could not parse body' );
					}
					else
					{
						callback( err );
